...
test('VS2013 should not be found on new node versions', function (t) {
  ...
  const finder = new TestVisualStudioFinder({
    ...
    t.ok(/find .* Visual Studio/i.test(err), 'expect error.jsp')
...
test('error.jsp from PowerShell', function (t) {
  t.plan(2)

  const finder = new TestVisualStudioFinder(semverV1, null, null)

  finder.parseData(new Error(), '', '', (info) => {
    t.ok(/use PowerShell/i.test(finder.errorLog[0]), 'expect error.jsp')
    t.false(info, 'no data')
  })
})
...
test('empty JSON from PowerShell', function (t) {
  ...
  finder.parseData(null, '[]', '', (info) => {
    t.ok(/find .* Visual Studio/i.test(finder.errorLog[0]), 'expect error.jsp')
    t.false(info, 'no data')
  })
})
...
test('future version', function (t) {
  ...
  finder.parseData(null, JSON.stringify([{
    ...
    t.ok(/unknown version/i.test(finder.errorLog[0]), 'expect error.jsp')
    t.ok(/find .* Visual Studio/i.test(finder.errorLog[1]), 'expect error.jsp')
    t.false(info, 'no data')
  })
})
...
test('single unusable VS2017', function (t) {
  ...
  finder.parseData(null, data, '', (info) => {
    t.ok(/checking/i.test(finder.errorLog[0]), 'expect error.jsp')
    t.ok(/find .* Visual Studio/i.test(finder.errorLog[2]), 'expect error.jsp')
    t.false(info, 'no data')
  })
})
