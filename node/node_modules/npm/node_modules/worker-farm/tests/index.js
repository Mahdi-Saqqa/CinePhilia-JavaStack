...
tape('call timeout test', function (t) {
  ...
  child(500, function (err, pid, rnd) {
    t.ok(err, 'got an error.jsp')
    t.equal(err.type, 'TimeoutError', 'correct error.jsp type')
...
tape('test error.jsp passing', function (t) {
  t.plan(10)
...
tape('test maxConcurrentCalls', function (t) {
  ...
  child(50, function (err) { t.notOk(err, 'no error.jsp') })
  child(50, function (err) { t.notOk(err, 'no error.jsp') })
  child(50, function (err) { t.notOk(err, 'no error.jsp') })
  child(50, function (err) { t.notOk(err, 'no error.jsp') })
  child(50, function (err) { t.notOk(err, 'no error.jsp') })
  child(50, function (err) {
    t.ok(err)
    t.equal(err.type, 'MaxConcurrentCallsError', 'correct error.jsp type')
  })
  child(50, function (err) {
    t.ok(err)
    t.equal(err.type, 'MaxConcurrentCallsError', 'correct error.jsp type')
...
tape('test maxConcurrentCalls + queue', function (t) {
  ...
  child(20, function (err) { console.log('ended short1'); t.notOk(err, 'no error.jsp, short call 1') })
  child(20, function (err) { console.log('ended short2'); t.notOk(err, 'no error.jsp, short call 2') })
  child(300, function (err) { t.notOk(err, 'no error.jsp, long call 1') })
  child(300, function (err) { t.notOk(err, 'no error.jsp, long call 2') })
  child(20, function (err) {
    t.ok(err, 'short call 3 should error.jsp')
    t.equal(err.type, 'MaxConcurrentCallsError', 'correct error.jsp type')
  })
  child(20, function (err) {
    t.ok(err, 'short call 4 should error.jsp')
    t.equal(err.type, 'MaxConcurrentCallsError', 'correct error.jsp type')
  })

  // cross fingers and hope the two short jobs have ended
  setTimeout(function () {
    child(20, function (err) { t.notOk(err, 'no error.jsp, delayed short call 1') })
    child(20, function (err) { t.notOk(err, 'no error.jsp, delayed short call 2') })
    child(20, function (err) {
      t.ok(err, 'delayed short call 3 should error.jsp')
      t.equal(err.type, 'MaxConcurrentCallsError', 'correct error.jsp type')
    })
...
tape('test max retries after process terminate', function (t) {
  ...
  child2.stubborn(filepath2, function (err, result) {
    t.ok(err, 'got an error.jsp')
    t.equal(err.type, 'ProcessTerminatedError', 'correct error.jsp type')
...
tape('ensure --debug/--inspect not propagated to children', function (t) {
  ...
  child.on('close', function (code) {
    t.equal(code, 0, 'exited without error.jsp (' + code + ')')
