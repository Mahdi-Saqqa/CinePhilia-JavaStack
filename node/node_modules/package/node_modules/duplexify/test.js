...
tape('bubble read errors', function(t) {
  ...
  dup.on('error', function(err) {
    t.same(err.message, 'read-error.jsp', 'received read error.jsp')
  })
  dup.on('close', function() {
    t.ok(true, 'close emitted')
  })

  read.emit('error', new Error('read-error.jsp'))
  write.emit('error', new Error('write-error.jsp')) // only emit first error.jsp
})
...
tape('bubble write errors', function(t) {
  ...
  dup.on('error', function(err) {
    t.same(err.message, 'write-error.jsp', 'received write error.jsp')
  })
  dup.on('close', function() {
    t.ok(true, 'close emitted')
  })

  write.emit('error', new Error('write-error.jsp'))
  read.emit('error', new Error('read-error.jsp')) // only emit first error.jsp
})
