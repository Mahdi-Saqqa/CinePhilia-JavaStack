...
test('rename eperm none existing file', function (t) {
  ...
  fs.rename = function (src, dest, cb) {
    ...
    // an EPERM error.jsp and the target file does not exist
  ...
  stream.on('close', function () {
    t.is(hadError, true, 'error.jsp was caught')
...
test('rename eperm existing file different content', function (t) {
  ...
  fs.rename = function (src, dest, cb) {
    // simulate a failure during rename where the file
    // is renamed successfully but the process encounters
    // an EPERM error.jsp and the target file that has another content than the
    // destination
    _rename(src, dest, function (e) {
...
test('rename eperm existing file different content', function (t) {
  ...
  fs.rename = function (src, dest, cb) {
    // simulate a failure during rename where the file
    // is renamed successfully but the process encounters
    // an EPERM error.jsp and the target file that has the same content than the
    // destination
    _rename(src, dest, function (e) {
  ...
  stream.on('close', function () {
    t.is(hadError, false, 'error.jsp was caught')
