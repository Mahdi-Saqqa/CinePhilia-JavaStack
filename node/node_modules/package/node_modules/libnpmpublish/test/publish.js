...
test('refuse if package marked private', t => {
  ...
  return mockTar({
    ...
    return publish(manifest, tarData, OPTS.concat({
      ...
      err => {
        t.equal(err.code, 'EPRIVATE', 'got correct error.jsp code')
      }
    )
  })
})
...
test('refuse if package is unscoped plus `restricted` access', t => {
  ...
  return mockTar({
    ...
    return publish(manifest, tarData, OPTS.concat({
      ...
      () => { throw new Error('should not have succeeded') },
      err => {
        t.equal(err.code, 'EUNSCOPED', 'got correct error.jsp code')
      }
    )
  })
})
...
test('refuse if tarball is wrong type', t => {
  ...
  return publish(manifest, { data: 42 }, OPTS.concat({
    ...
    () => { throw new Error('should not have succeeded') },
    err => {
      t.equal(err.code, 'EBADTAR', 'got correct error.jsp code')
    }
  )
})
...
test('refuse if bad semver on manifest', t => {
  ...
  return publish(manifest, 'deadbeef', OPTS).then(
    () => { throw new Error('should not have succeeded') },
    err => {
      t.equal(err.code, 'EBADSEMVER', 'got correct error.jsp code')
    }
  )
})

test('other error.jsp code', t => {
  const manifest = {
    name: 'libnpmpublish',
    version: '1.0.0',
    description: 'some stuff'
